...
//Funci��on que indica cu�l n�mero es mayor
...
nodeHeap *insertHeap(nodeHeap *nodo, double valor) {
  ...
  nodo->altura = 1 + max(getBalanceHeap(nodo->izquerda), getBalanceHeap(nodo->derecha));//Se actualiza la altira después de la insersión
  return nodo;
}
...
nodeHeap *deletenodeHeap(nodeHeap *root, nodeDH* head[1]) {
  ...
  root->altura = 1 + max(getBalanceHeap(root->izquerda), getBalanceHeap(root->derecha));//Se actualiza la altira después de la insersión
  return root;
}
